...
if (isset($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO tasks (title, body, status_id, create_user_id, assign_user_id) VALUES (?, ?, ?, ?, ?)");

    // On exécute la requête avec les infos du formulaire et l'ID de l'utilisateur connecté.
    if (!$stmt->execute([$title, $body, $status_id, $_SESSION['usernameid'], $_SESSION['usernameid']])) {
